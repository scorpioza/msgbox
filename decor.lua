...
function txt:new(v)
	...
	if type(text) == 'function' then
		-- ZEID - нужна возможность вызова пользовательской функции
...
function decor:render()
	...
	-- ZEID - не надо перекрашивать фон
...
function decor:load()
	...
	-- ZEID - пока не нужно возвращать сохраненное состояние
	-- for _, v in pairs(self.objects) do
		-- self:new(v)
	-- end
end
...
std.mod_start(
	function(load)
		...
		if load then
			-- ZEID -- не нужно перекрашивать фон
...
-- ZEID - удаление и обновление элементов
function DR(nm)
	decor:remove(nm)
end

function decor:remove(name)
	local tt = self.objects[name].type
	self[tt]:delete(self.objects[name])
	self.objects[name] = nil
end
...
function DU(nm)
	return decor:update(nm)
end

function decor:update(name)
	local tt = self.objects[name].type
	self[tt]:delete(self.objects[name])
	return decor:new(self.objects[name])
end
...
std.mod_save(function(fp)
	-- ZEID - пока не нужно сохранять состояние
